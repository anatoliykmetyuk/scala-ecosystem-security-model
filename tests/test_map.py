...
def test_dependency_coverage_layers_and_incoming_exposure(tmp_path: Path) -> None:
    ...
    with sync_playwright() as p:
        ...
        expect(page.locator("#coverage-legend")).to_be_hidden()
        expect(page.locator("#unknown-legend")).to_be_visible()
        page.get_by_role("button", name="Compromise this project", exact=True).click()
        ...
        expect(page.locator("#coverage-legend")).to_be_visible()
        expect(page.locator("#unknown-legend")).to_be_hidden()
        page.get_by_role("button", name="Reset", exact=True).click()

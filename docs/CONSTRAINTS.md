...
| Seed projects | At most 100 | Enforced by seed configuration validation and selection. Current frozen seed contains 100 repositories. |
| Projects per main category | At most 10 allocated to each main Awesome Scala section | Selection interleaves source-ranked child lists and deduplicates repositories. A project can retain memberships in several sections without consuming several allocations. |
...
| Universe size | At most 2,000 candidate coordinates across at most 100 projects | Derived from the project and artifact caps, not an additional independent setting. Current offline expansion yields 803 distinct candidates; publication and latest-release checks can reduce the usable set. |
| Recursive dependencies per project | No separate numeric cap | Traversal stays within the selected universe and hop limit. At repository level, a seed can reach at most 99 other repositories with the current 100-project seed. That is a derived ceiling, not a limit on dependency declarations or artifact versions. |
| Total recursive dependency resolutions | No separate numeric cap | Specific historical versions of the same coordinate can require separate resolution. Coordinate and project caps do not bound requests to 803 or 2,000. Duplicate version nodes are fetched once per collection traversal; cached responses can be reused explicitly. |

...
A project is a GitHub repository; artifacts are Maven group/artifact coordinates. Seed schema 2 stores module families without cross-build suffixes and a shared matrix. The current matrix permits JVM coordinates for Scala 2.13 and 3 only. Generated combinations are intersected with the refreshed Scaladex inventory; only published matches become target coordinates. Availability is recorded per candidate in SQLite. Neither a missing variant nor an unavailable inventory triggers an unverified fallback. Historical versions of selected coordinates still match; Scala 2.12, JS/Native, full-compiler-version and unsuffixed coordinates are outside the selected target matrix. The matrix restricts target discovery and seed starting roots, not the exact versions of intermediate dependencies or Java consumers. When a path ends at a nonselected artifact of a seed repository, it does not qualify that repository as a target. Multiple repository claims are reconciled against ecosyste.ms package repository metadata; unresolved ownership is a reported gap.
